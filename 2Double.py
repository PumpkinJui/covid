...
con = [0,1,1,17,25,77,144,54,19,8,226,188,87,113,# 27~10
       ...
       58,105,55,71,94,66,66,53,68,66,# 21~30
       51,66,40
       ]
...
conall = [0,1,2,19,44,121,265,319,338,346,572,723,794,885,
          ...
          894,868,734,705,575,470,469,450,442,445,
          438,466,460
          ]
...
non = [0,0,0,6,5,21,225,239,441,651,371,376,499,574,
       ...
       422,315,243,244,152,176,139,169,125,99,
       133,96,107
       ]
...
nonall = [0,0,0,6,11,29,248,453,880,1525,1765,2066,2538,3070,
          ...
          7502,6431,4701,4159,662,722,824,# 21~27
          824+169-697-1,824+169-697-1+125-1055-2,824+169-697-1+125-1055-2+99-143,# 28~30
          824+169-697-1+125-1055-2+99-143+133-82,# 1
          824+169-697-1+125-1055-2+99-143+133-82+96-80,# 2
          824+169-697-1+125-1055-2+99-143+133-82+96-80+107-97
          ]# 我一定是个傻逼，在源代码里列这么长的算式
...
conn = ax1.plot(x,con,marker='o',c='orange',label='确诊')
cont = ax1.plot(x,conall,marker='o',c='red',label='现有确诊')
nonn = ax2.plot(x,non,marker='o',c='purple',label='无症状')
nont = ax2.plot(x,nonall,marker='o',c='blue',label='现有无症状')# 折线图

lines = conn + cont + nonn + nont
labels = [i.get_label() for i in lines]
plt.legend(lines,labels,loc='best')# 显示标签

ax1.grid(True,linestyle='--',alpha=0.5)
ax2.grid(True,linestyle='--',alpha=0.5)# 显示网格
...
ax1.tick_params(axis='y',labelcolor='#E2492F')
ax2.tick_params(axis='y',labelcolor='#0F4C81')# 更改坐标轴数字颜色
ax1.set_xlabel('日期',size=14)
ax1.set_ylabel('确诊病例数',size=14,color='#E2492F')
ax2.set_ylabel('无症状病例数',size=14,color='#0F4C81',rotation=270,labelpad=20)# 显示坐标轴标签

day = str(int(strftime("%y%m%d",localtime(time()-86400))))# 计算昨天几号
h = int(strftime("%H",localtime()))# 计算现在几点
plt.savefig(day + '_Double.png')# 保存图片

if demo('220926',day) != len(date) and h >= 9:# 内蒙古卫健委在九点更新前一天的数据
    txt = '当前数据已过期，最新为'+day[:-3:-1]+'，现为'+str(date[-1])
    ax2.text(20,8000,txt,color='#88B04B')
elif demo('220926',day) != len(date):# 到了第二天但没到九点，暂无数据
    txt = '当前数据为 '+str(int(day)-1)+' 的最新数据'
    ax2.text(20,8000,txt,color='green')
else:# 当天
    txt = '当前数据为 '+day+' 的最新数据'
    ax2.text(20,8000,txt,color='green')
